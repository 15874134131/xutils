...
/**
 * Created by xu Administrator on 2018/12/4.
 * 中心点点击波纹。建议直接使用官方的CardView ，
 */

public class XCenterSpreadLayout extends RelativeLayout {
     ...
     * 渐变的背景色
     */
    private int mCirclelColor = Color.RED;
    /**
     * 每次重绘时半径的增幅
     */
     ...
     * @param context
     */
    public XCenterSpreadLayout(Context context) {
        this(context, null);
    }

    public XCenterSpreadLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public XCenterSpreadLayout(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void initTypedArray(Context context, AttributeSet attrs) {
        final TypedArray typedArray = context.obtainStyledAttributes(attrs,
                R.styleable.XCenterSpreadLayout);
        mCirclelColor = typedArray.getColor(R.styleable.XCenterSpreadLayout_mycolor, Color.LTGRAY);
        mDuration = typedArray.getInteger(R.styleable.XCenterSpreadLayout_duration,
                DEFAULT_DURATION);
        mFrameRate = typedArray
                .getInteger(R.styleable.XCenterSpreadLayout_framerate, DEFAULT_FRAME_RATE);
        mColorAlpha = typedArray.getInteger(R.styleable.XCenterSpreadLayout_myalpha, DEFAULT_ALPHA);
        mCircleScale = typedArray.getFloat(R.styleable.XCenterSpreadLayout_myscale, DEFAULT_SCALE);
        typedArray.recycle();
    }
